...
func TestBundles(t *testing.T) {
	...
	tx := ofActor2.Switch.NewTransaction(int16(Atomic))
	err := tx.Begin()
	...
	err = tx.AddFlows([]*Flow{flow1, flow2})
	...
	tx2 := ofActor2.Switch.NewTransaction(int16(Atomic))
	err = tx2.Begin()
	require.Nil(t, err, fmt.Sprintf("Failed to create transaction: %v", err))
	flow3 := createFlow(t, "22:11:11:11:11:13", "192.168.2.13")
	err = tx2.AddFlows([]*Flow{flow3})
	require.Nil(t, err, fmt.Sprintf("Failed to add flowMod into transaction: %v", err))
	...
	tx3 := ofActor2.Switch.NewTransaction(int16(Atomic))
	err = tx3.Begin()
	require.Nil(t, err, fmt.Sprintf("Failed to create transaction: %v", err))
