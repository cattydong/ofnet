...
// NewTransaction->Begin->AddFlows->Complete->Commit. Client could cancel the messages by calling Abort after the
...
const (
	Atomic  = openflow13.OFPBCT_ATOMIC
	Ordered = openflow13.OFPBCT_ORDERED
)
...
func (self *OFSwitch) NewTransaction(flag int16) *Transaction {
	tx := new(Transaction)
...
// AddFlows adds messages in the bundle.
func (tx *Transaction) AddFlows(flows []*Flow) error {
	for _, f := range flows {
		message, err := tx.createBundleAddMessage(f)
		if err != nil {
			return err
		}
		tx.lock.Lock()
